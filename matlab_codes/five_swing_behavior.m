% Parameters
n = 0.25; % ratio of maximum acceleration to g
k = 100; % control gain
m = 0.01; % mass (kg)
...
dtheta0 = 0.001; % initial angular velocity (rad/s)
...
% Calculating initial energy and print it
E_initial = 0.5 * J * dtheta0^2 + m * g * l * (cos(theta0) - 1);
fprintf('Initial Energy E: %.2f J\n', E_initial);

% Calculate control signal over time and print initial value
...
for i = 1:length(t)
    ...
    u_unsat = k * (E - E0) * sign(dtheta * cos(theta));
    % Applying the sat fuction in equation 8
    u_control_g_force(i) = min(max(u_unsat / g, -n), n); % Normalizing by g to express in terms of g-force
    % Printing the first value of control input
    if i == 1
        fprintf('Initial Control Signal: %.2f g\n', u_control_g_force(i));
    end
    
    % Stopping the applied force once the desired energy is reached
    if E >= E0 && i > 1
...
time = linspace(0, simTime, length(theta));  % Creating a time vector for sim time in seconds

% Getting the length of theta
len_theta = length(theta);

% Defining x and y positions as a function of theta
x_pos = @(t) sin(interp1(time, theta, t));   % Interpolating theta for continuous time t
y_pos = @(t) cos(interp1(time, theta, t));  % Interpolating theta for continuous time t
...
% Plotting the line connecting the origin to the pendulum bob
fanimator(@(t) plot([0 x_pos(t)], [0 y_pos(t)], 'k-'), 'AnimationRange', [0 simTime], 'FrameRate', len_theta/simTime);

% Displaying the timer as text (t maps to time directly)
